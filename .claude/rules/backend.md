...
- **Windows video (wid embedding, PoC)**: `mpv_engine/video_layer_win.rs` creates a disabled child HWND at the bottom of the sibling z-order (below WebView2) and hands it to deck 0 via `wid` + `vo=gpu` — mpv renders itself, no render thread. Presentation is forced to **bit-blt** (`d3d11-flip=no`): DWM does not composite flip-model swapchains in a child of our layered (`transparent: true`) top-level window — mpv renders frames nobody displays (found on real hardware). Enabled whenever the engine is selected; the `WINVIDEO:` logs are temporary validation scaffolding until on-screen rendering is confirmed. Debug hooks: `VIBOPLR_MPV_OPTS="key=val;…"` (raw options at deck creation), `VIBOPLR_WIN_VIDEO_VO` (vo override at wire time).
