...
- **Single-track (canonical) → `DownloadModal`:**
  ...
  - **Local tracks → "Upgrade from {X}…":** a local library track gets no "Download…" (`decideDownload` maps `local` → none), but its context menu offers each **interactive** plugin download provider as an "Upgrade from {X}…" entry (an "Upgrade" submenu when there are several) — on both the library track menu and a single-selection queue entry with a `lib:` key (`buildUpgradeSpecs` in `buildContextMenuSpecs.tsx`). These open the same `DownloadModal`, which enters upgrade mode on the track's `file://` uri. Non-interactive providers are deliberately **not** offered on local tracks: their path is a background fresh enqueue, which cannot replace a file in place.
