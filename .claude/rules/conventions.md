...
- **The `download-resolve-request` bridge stays** — mixtape export resolves remote tracks by emitting it and awaiting `download_resolve_response`; the frontend answers via `resolveTrackDownload` (built-in Subsonic first, then plugin providers in registration order, by-URI then by-metadata). That chain is now mixtape-only.
