...
- **useNowPlayingInfo.ts** — Resolves the cycling Now Playing info items for the current track: built-in resolvers (Artist · Album / Artist / Album / Plays · Rank / Source / Quality / Duration / Tags) merged with plugin items (`plugins.nowPlayingInfoItems`), fetched with a 5s timeout + generation guard. Returns `availableItems` (context-menu checklist) + `resolvedItems` (display). Exports pure helpers `isNowPlayingItemSelected` / `formatPlays` / `formatSource` / `formatQuality` / `formatTags` / `nextCycleIndex` (unit-tested). Selection persisted by App.tsx as `nowPlayingInfoSelection`.
