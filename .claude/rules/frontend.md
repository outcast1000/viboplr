...
- **NowPlayingBar.tsx** — Footer playback controls. Full mode: seek bar (waveform or segmented), track info with like/dislike, transport controls, queue mode, auto-continue, volume. Mini mode: compact draggable bar with art, title/artist, play controls. Rank badges for top-100 tracks. Scrobble indicator checkmark. Album art resolved async via `currentTrack` effect in `App.tsx` — same priority chain as queue (see `queue.md` "Image Resolution" section). The artist/album line under the title is the dynamic, cycling **Now Playing info** section, rendered by `NowPlayingInfoCycler` (both mini + full bar).
- **NowPlayingInfoCycler.tsx** — Presentational cycling line for the Now Playing info section. Auto-rotates the host-resolved enabled items (5s) with the shared `slide-text-enter` animation; one item → static. `plain` mode (mini player) renders text only; full-bar mode renders clickable artist/album links + rank badge. Items come from `useNowPlayingInfo`; selection is toggled via the native context menu (see `plugins.md` "api.nowPlayingInfo").
...
- **useNowPlayingInfo.ts** — Resolves the cycling Now Playing info items for the current track: built-in resolvers (Artist · Album / Artist / Album / Plays · Rank) merged with plugin items (`plugins.nowPlayingInfoItems`), fetched with a 5s timeout + generation guard. Returns `availableItems` (context-menu checklist) + `resolvedItems` (display). Exports pure helpers `isNowPlayingItemSelected` / `formatPlays` / `nextCycleIndex` (unit-tested). Selection persisted by App.tsx as `nowPlayingInfoSelection`.
