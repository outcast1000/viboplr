...
`PluginTrack`: `{ path?, title, artist_name?, album_title?, duration_secs?, track_number?, image_url? }`. `image_url` is shown in the now playing bar and queue when no library image exists. There is deliberately **no** `format` field — see "Video-vs-audio" below.
...
`DownloadResolveResult`: `{ url, headers?, metadata?: { title, artist, album, trackNumber, year, genre, coverUrl }, ext? }`. `ext` overrides the saved file extension for the requested format; `ext: "auto"` tells the backend to sniff the container from the downloaded bytes (for originals of unknown format). **A download provider should return a concrete `ext` whenever it knows the container** — it's the provider's job to name the file, and it's the only fully reliable signal. `"auto"` sniffing is honored **only on the background queue path** (`enqueue_download` → multi-track/batch downloads); the interactive `DownloadModal` (single-track downloads) resolves the extension *before* the bytes arrive and never sniffs, so `"auto"` there falls back to the format's default extension (or `.bin` when the provider also declared no `onGetQualities`). Don't rely on `"auto"` for single-track downloads — return the real `ext`.
...
**The host handles this itself, and a plugin whose scheme resolves to a local file needs to do nothing.** The by-URI chain entry already knows the real path (`attributedSourceUrl`, which derives `file://<engineSource.path>` even when the resolver reported no `sourceUrl`); it runs that through `videoContainerFromPath` and patches `format` onto the track. Resolution completes *before* any element or engine is chosen, so the patched classification is what routes playback — the same `patch: { format }` channel the prefer-video pass uses. **There is no `PluginTrack.format` field, deliberately**: a field every plugin author must remember would leave already-released plugins broken, and the plugin's claim is a worse source of truth than the file.

A plugin whose stream is **not** a local file has to say so, because there is no filename to read:

- **`onStreamResolve` (metadata):** return `video: true` in answer to `opts.preferVideo`. The host stamps `format: "mp4"` and plays it in the theater; `video: false` on an already-video track downgrades it to audio and notifies. This is the documented channel and the only one that participates in the prefer-video pass.
- **An extension on your own URI.** `isVideoTrack`'s path fallback sees it, which is what `ytdlp://` does: `encodeRef` percent-escapes every dot in the encoded page URL (`%2E`) and appends a literal `.mp4` for video refs, then `decodeRef` parses the suffix back off on resolve. Workable, but it is a side channel — prefer `video: true` for anything new, and if you do this, escape the other dots or an audio ref will false-trip the same detector.

`onResolveStreamByUri` has **no** `video` flag and never gets `opts.preferVideo`: by then the id itself is the request, so the answer is either a file the host can classify or a `kind`-tagged candidate list.
...
Providers are prioritized by internal hardcoded defaults (in `usePlugins.ts`); unknown plugins are added last. The user's Settings > Providers configuration applies to actual resolution, not just display: a disabled provider is skipped by the auto-resolve chain, `decideDownload`, and the context menus, and the chain walks providers in the user's priority order (the enable toggle survives restarts — `sync_download_providers` never resets it). Each resolver call is bounded by **60 seconds of silence** — an *idle* timeout, not a deadline (`RESOLVE_IDLE_TIMEOUT_MS` in `useDownloadOrchestration.ts`), with the provider's `reportProgress` calls as the liveness signal. It has to be idle-based because a download resolver like yt-dlp fetches the whole file before answering, so its resolve legitimately runs for minutes: the old fixed races (60s by URI, **10s** by metadata) failed every real video download and reported it as "no download provider could resolve this track". A provider that reports nothing still gets the full budget once. The backend's wait (`recv_timeout` in `lib.rs`) is correspondingly not a duration bound but a dead-webview guard, so it sits far above any real download. The built-in Subsonic provider handles `subsonic://` URIs natively and is always enabled and first.
