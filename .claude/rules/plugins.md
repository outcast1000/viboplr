...
**Picking one:** the Now Playing view's **⋯ button** (top-right) opens a native menu with the picker, the lyrics toggle, and "Fullscreen visualizer"; right-clicking anywhere in the view opens the same menu, and Settings → Playback still has a select. Declaring a visualizer is enough to appear in all three — no extra registration.
