...
- `openUrl(url)` — open in system browser. The opener scope permits http(s)/mailto/tel **plus `roadie://`** (`capabilities/default.json`): a plugin can hand off to [Roadie](https://github.com/outcast1000/roadie), the standalone tool manager that installs and runs daemons such as slskd. The plugin then reads Roadie's loopback status API (`http://127.0.0.1:47630/v1/tools/<name>`) with `fetch` and receives the connection through a scoped deep link back (`viboplr://plugin/<pluginId>/roadie?status=connected`). No other custom scheme is openable, and Viboplr itself never installs or runs those tools — see `feedback-no-host-managed-daemons`.
