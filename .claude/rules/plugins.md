...
- **By metadata:** a plugin accepts arbitrary `(title, artistName, albumName, durationSecs, format)` via `onResolveByMetadata`. Consumed by `decideDownload`'s metadata closure (a stream-resolver win with no native URI) and by mixtape export's resolve round-trip — the background auto-download chain that used to walk these is gone.
