...
- `getHistory(opts?)` / `getMostPlayed(opts?)` / `getMostPlayedArtists(opts?)` — `opts.days` switches each to the rolling-window variant. `getMostPlayedArtists` returns `{ history_artist_id, play_count, track_count, display_name, rank }[]`. History stores no album, so the two track-returning calls resolve one from the library per row and report it as `display_album` + `display_album_artist` (the album's own artist — what an album cover is keyed by; both null when nothing matches). The resolution is one batched lookup through the normalized-title index, not a scan.
- `getHistoryPlayCount()` / `getHistoryPlaysPage(opts?)` — total play count + a cheap, **album-free**, keyset-paginated page of raw plays (`{ beforeTs?, beforeId?, limit? }`; pass the previous page's last row to advance). Use these to stream a large history in chunks rather than pulling it all via `getHistory`, which returns the richer per-row shape.
...
`NowPlayingInfoResult`: `{ status: "ok", text } | { status: "empty" } | { status: "error", message? }`. `empty`/`error`/timeout simply hide the item for that track (no error indicator). Built-in items contributed by the core app are **Artist · Album**, **Artist**, **Album**, **Plays · Rank** (play count + chart rank from history, in one item), **Source** (Local / Subsonic / Web / scheme), **Quality** (for video: codec · resolution · fps via the mpv engine's live facts; for audio: format · sample rate · bit depth, or bitrate — from `get_audio_properties_by_path` for a file on disk, since only the file states its real bit depth, falling back to the engine's live facts for anything streamed), **Duration**, **Tags** (`#`-prefixed track tags, resolved via `find_track_by_metadata` → `get_tags_for_track`), **Synced Lyrics** (the line *currently being sung* in quotes, tracking playback position), and **Plain Lyrics** (one line of unsynced lyrics in quotes, stable per track); the Last.fm plugin contributes **Listeners** (listener count + plays-per-listener average — Last.fm playcounts read "plays" in the UI, never "scrobbles"; its item id stays `scrobbles` because persisted selection/order key on it). The two lyrics items reuse the cached lyrics info-type (via `useLyrics`) and only fetch when enabled; each appears only when that kind of lyrics exists for the track (synced vs. plain), otherwise it's hidden for that track. The Synced Lyrics item additionally drops out of the cycle during intros and instrumental gaps — when no line is actively being sung (before the first timestamp, or on a blank LRC gap line) it resolves to nothing rather than lingering on the last sung line (`activeSyncedLine` in `utils/lyrics.ts`). Each item declares its own `defaultEnabled`; **Artist · Album**, **Synced Lyrics**, and the Last.fm **Listeners** item are on by default — everything else (including **Tags** and **Plain Lyrics**) is opt-in. Each item also has a built-in per-type style (skin-token-backed), a time-of-persistence multiplier, and a user-set priority (its position in the cycle) — the latter two set in Settings → Playback → "Now playing info" (persisted as `nowPlayingInfoPersistence` / `nowPlayingInfoOrder`); see `ui.md` "Now Playing Bar". A plugin item's `priority` therefore only decides where it *first* appears (after the built-ins, ordered among plugin items); the user can move it anywhere afterwards. When a plugin is deactivated/reloaded, `usePlugins` drops its items + handlers automatically. One dwell option is **"On request"**: the item never sits in the steady rotation and instead preempts the line for up to ~10s whenever its resolved content *changes or (re)appears* (see `ui.md` "Now Playing Bar"). Since a plugin item resolves once per track, an on-request plugin item flashes once when its text lands at track start; the built-in Synced Lyrics item — which re-resolves per sung line — uses it as its default for karaoke-style behavior. There is no push API (`requestShow`-style) yet; if a plugin needs mid-track requests, that's the extension point to add.
...
- `openUrl(url)` — open in system browser. The opener scope permits http(s)/mailto/tel **plus `roadie://`** (`capabilities/default.json`): a plugin can hand off to the *desktop* [Roadie](https://github.com/outcast1000/roadie) app, the standalone tool manager that installs and runs daemons such as slskd. That scope is kept for plugins that still use the desktop hand-off (slskd ≤ 0.7.0). The current path is the **Roadie CLI as a managed dependency**: the plugin checks `api.system.getDependency("roadie")`, offers the host's install modal via `api.ui.requestAction("require-dependency", { name: "roadie", feature })`, and runs `api.system.exec("roadie", ["--as", "Viboplr", "tool", …])`. Roadie asks the user in its own dialog before any install, and hands the plugin a per-consumer key with `tool connection <tool> --consumer <id>`. No other custom scheme is openable, and Viboplr itself never installs or runs those tools — see `feedback-no-host-managed-daemons` and backend.md "External Binary Dependencies".
...
- `trashPath(collectionId, relativePath)` — move a file or folder **the plugin put inside a local collection** to the OS trash (permanent delete on a network share, same rule as `delete_tracks`) and drop any library rows under it. Exists for exactly one job: cleaning up a download that failed or was cancelled — a torrent client preallocates every file to full size, so a failed run's leftovers look complete and would be scanned in as music, and `api.storage.files` is rooted in the plugin's own directory so it cannot reach them. Rust owns every check (`plugin_trash_collection_path`: `parse_relative_dir` + `ensure_within_root` from the assistant write layer, local collections only, the root itself refused, target must exist); the frontend bridge is one invoke. Guard with a `typeof` check for older hosts.
...
- There is **no `enqueue`** — the background download queue (`enqueue_download` + its worker) was removed along with the host's batch menu items. A plugin that wants a download UI opens the host modal via `api.ui.requestAction("download-tracks", { providerId, providerName, tracks })` (single track → `SingleTrackDownload`, several → `MultiTrackDownload`), or downloads itself and lands the files in a local collection (`api.collections.resync`).
  - **Replacing a library file:** a *single* track may carry `libraryTrackId` (the library row this download is meant to replace) alongside its `uri`. The host reads the row and, when it is a local `file://` track, opens the modal's in-place **upgrade** flow — `download_preview` compare → Replace / Save as copy / Cancel — while still resolving the plugin's `uri` through the provider. A row that isn't local falls back to the ordinary fresh download, and an older host ignores the field. Canonical consumer: the Soulseek plugin's "Upgrade with Soulseek…" → "Replace in library…". It is the only plugin route to an in-place upgrade; a plugin must never move or delete library files itself.
...
### api.system
- `exec(program, args?, opts?)` — run a subprocess, returns `{ exitCode, stdout, stderr }`. **Allow-list only:** currently `yt-dlp`, `ffmpeg`, `rqbit` and `roadie` (the dependency registry; `roadie` always runs with Viboplr's own `--data-dir` prepended by the host — see backend.md "External Binary Dependencies"). `opts.cwd` defaults to the app data directory. `opts.onStart(handle)` fires synchronously before the spawn with a `{ cancel() }` handle — the plugin's own stop button for a long run it started **outside** any resolve (a torrent download from its own view), where no host Cancel exists; the exec then rejects with `"Cancelled"` exactly as a host cancel would. Passing it takes the exec-id path, so an `onStart`-only exec is piped like a streamed one. Feature-detect: an older host never calls it. `opts.onOutput(line, stream)` streams the child's output live, split on `\n` **and** `\r` so a CLI that redraws one progress line still reports (yt-dlp needs `--newline` + `--progress` anyway, since a redraw never completes a line); the resolved `stdout`/`stderr` are still the complete, **verbatim** text, so an existing parse is unaffected. An exec started **inside a download resolve** is killed when the user cancels that download and the promise then rejects with **`"Cancelled"`** — rethrow that verbatim rather than mapping it to a failure message, because the host recognises its own cancellation and stays silent (see `api.downloads.reportProgress`). Execs outside a resolve with neither `onOutput` nor `onStart` (a version probe) take the original unpiped path and are not cancellable.
...
2. Walk the user-ordered stream resolver list. The **built-in Library resolver** (`createLibraryStreamResolver` in `streamResolvers.ts`) sits first by default: it fetches *every* library copy of the metadata match via `find_tracks_by_metadata` (ordered local > subsonic > other), verifies a local copy still exists on disk (`file_exists`), and answers with the first playable one — so a row whose file is gone falls through to the network copy rather than ending the lookup. Plugin resolvers follow; each is called with `(title, artistName, albumName, durationSecs)` and has 60 seconds to return `{ url, label } | null` (resolvers like YouTube shell out to `yt-dlp`, which can be slow). For a **path-less** track — a Home track-row carrying only title + artist — there is no step 1, so the Library resolver is the first entry; its `patch` (path + format) is also what lets the play path classify such a track from the file that was matched.
3. First success wins. Failures fall through to the next resolver. `notify()` surfaces fallback info to the user.

**Prefer-video pass:** with the "Prefer video" toggle on, a second, video-only pass of the resolver list is prepended ahead of everything (a plugin owning the track's own scheme excluded). Only a `video: true` answer counts there; a miss falls through to the normal chain and the track plays as audio, with a notify. The **Library resolver participates**: `preferVideo` on its `resolve` opts restricts it to video copies of the match, so a user's own music-video file beats fetching one from the network — with no video copy it answers null. Plugin resolvers get the prefer-video hint through `invokeStreamResolve` and ignore that opts field.

**The track's own source runs before any library copy, and the order is load-bearing.** A local-copy shortcut used to run first, written when every non-`file://` scheme was a genuine network stream (`subsonic://`, `tidal://`), where substituting a local file is a plain win. But `isRemoteScheme` is only "not `file://`", so a plugin scheme that resolves to a file *on the same disk* — `qbt://` — was caught by the same rule, and there the shortcut wins nothing: it outranked the exact file the user clicked with a metadata match, which is fuzzy about **which** copy. Clicking `05. Nothingman.mp3` inside a Vitalogy torrent played `Pearl Jam - Nothingman.mp3` out of an unrelated compilation folder — a different recording, silently. Known cost of the order: a track whose own source is dead or slow (a taken-down video, an unreachable server) waits for that to fail before the local copy plays. (There is no separate pinned pre-chain library entry any more — it duplicated the Library resolver's lookup with subtly different rules; the resolver is the one place library copies are served, which also means demoting/disabling it in Settings > Providers genuinely demotes library copies.)

**A chain entry must fail during *resolve*, not during playback.** The chain only advances when `resolve()` throws (or the resolver returns null); a source that resolves "successfully" and then won't load is a dead end — the retry re-resolves the same entry and gives up. That is why the Library resolver verifies a local file with `file_exists` before handing back a path: a library row outlives its file, and without the check a stale row killed playback for a track whose real copy was sitting right there. Any new entry that can go stale needs the same treatment.
...
**`state.loading` — the host is resolving, so don't believe its position yet.** Optional (`undefined` on a host older than it, which means *unknown*, not *idle*). True from the start of a play or a cued load until the source is installed — a whole network round-trip for a remote track. During that window the host reports the track and position it was **asked for**, not one it has reached, so a visualizer that writes a position and waits for the host to echo it back sees its own request echoed on the very next frame and concludes it has landed. It hasn't: the engine then starts reporting from its own clock, which begins at **0** on a fresh load and only reaches the seek once the stream has buffered. The vinyl deck's arm shipped that bug twice — first as a wobble on every cue, then, after a timeout-based hold, as a wobble on remote tracks only. A timeout cannot fix it, because the wait being measured is a network's, not a clock's; ignoring the echo while `loading` is true is the only test that separates the host's intent from its arrival, since both report identical numbers.
