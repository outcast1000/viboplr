...
**Header actions:** two visible toggle/action buttons plus a `⋯` overflow menu. **Randomize** (one-shot reorder; disabled in Repeat One or under 2 tracks — it moved here from the transport bars' `QueueModeGroup`, next to the list it reorders) and **Prefer video** (the ONLY control for the mode — it left Settings → Playback and this menu; while on, an accent-tinted banner row at the head of the list says "Prefer video is on…" with a **Turn off** button, so the mode is visible and cancellable at its point of effect — a mode that changes what every play does must not hide behind a ⋯). The `⋯` native menu holds the remaining queue-level actions — Load playlist…, Save ▸ (Save as Playlist / Export as M3U), Share ▸ (Publish hosted source… / Save as file (.mixtape)…), and Clear playlist; new *menu-appropriate* queue-level actions still go there, while a persistent **mode** gets a visible button + banner like Prefer video. The spec list is built by the pure `buildQueueHeaderMenuSpecs()` in `src/contextMenu/` — `QueuePanel` only supplies the anchor rect and calls `showNativeMenu`. Keep it that way: a native menu has no DOM, so the builder is the only place its items can be asserted (`queueHeaderMenu.test.ts`; the header buttons and banner are DOM and are pinned by `tests/e2e/specs/queue-url.test.js`).
