...
**The bar keeps both buttons here.** `onToggleFullscreen` and `onToggleQueue` are both **optional** on `FullscreenControls`, and audio fullscreen now passes both. **Exit** — the restore control belongs at the right end of the control bar in *every* fullscreen, so it is where the windowed bar's fullscreen button just was. It does duplicate the surface's corner row, and that is accepted: the corner row fades with the artwork and reads as part of the presentation, while the bar is the transport. **Queue** pins the edge-revealed drawer open (pressed state while the drawer is up, however summoned) — it was dropped for a while as "a second route to the edge gesture", but the gesture is invisible: nothing teaches it, issue #123's reporter found the drawer and still read it as broken, and a keyboard user can't perform a pointer gesture at all. The button is the discoverable route; the gesture stays the fast path. In fullscreen **Cmd/Ctrl+P** toggles the same pin (via `handleToggleQueueCollapsed`'s fullscreen branch — the persisted collapsed flag is never touched there). Video fullscreen passes both too (no corner row, no edge gesture).

**Queue drawer (edge-revealed, pinnable).** The root `.app` gains `audio-fs-open`, and `QueuePanel.css` turns the panel from a grid column into a fixed drawer: parked at `translateX(100%)`, `z-index: 1000` (above the overlay's 999, below the auto-continue popover at 1100 and modals at 2000), spanning `top: 52px` to `bottom: var(--now-playing-h)`. `App.tsx` reveals it — `fsQueueVisible = fsQueueRevealed || fsQueuePinned || fsQueueMenuOpen`, all live only while fullscreen is up — and the root gains `fs-queue-revealed`, which slides it in. `fsQueueRevealed` is the pointer's edge-hover, `fsQueuePinned` the bar Queue button / Cmd+P toggle, `fsQueueMenuOpen` a hold while a queue-item native context menu is open (the menu can extend past the drawer's left edge, so the first mousemove after it closes could land outside and hide the drawer on the heels of the action just picked — `buildAndShowNativeMenu` returns the menu-lifetime promise for exactly this). The drawer paints **translucent** (`rgba(var(--bg-primary-rgb), 0.78)` + `backdrop-filter: blur`) rather than the grid panel's opaque `--bg-secondary`, so it reads as part of the fullscreen presentation instead of windowed chrome — still skin-token-driven, since an always-dark scrim would invert row text on light skins. On each reveal the panel scrolls to the playing track (the `fsRevealed` prop, folded into the existing scroll-on-uncollapse effect) — the parked drawer keeps its scroll position, so it would otherwise open on wherever the user last browsed.
...
- **No reveal/hide while a mouse button is held** (`e.buttons !== 0`): a drag — reordering rows, or pulling the width-resize handle — legitimately takes the pointer outside the drawer mid-gesture, and hiding it under the drag drops the very rows being moved.
