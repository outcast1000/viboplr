...
**Filmstrip seek bar** (`FilmstripSeekBar.tsx` / `.css`) — the seek surface for a video with a storyboard, on both the now-playing bar (34px) and `FullscreenControls` (28px), **expanding to 72px on hover** (see below). The frames *are* the content, so four rules hold and each one replaced a bug:
