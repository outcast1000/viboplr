...
Two states:
- **Expanded:** Header (title + **Randomize** and **Prefer video** buttons + a `⋯` overflow native menu: load / save ▸ / share ▸ / clear) + scrollable queue list + info bar (count + duration). While Prefer video is on, an accent-tinted banner row at the head of the list announces the mode with a **Turn off** button (see queue.md "Header actions").
...
- Right: queue mode (normal/repeat-all/repeat-one), auto-continue, equalizer, volume, **fullscreen** (Randomize moved to the queue panel header)
...
**One surface, two chromes.** The same component renders the in-grid view *and* the audio fullscreen overlay (`variant="fullscreen"`, see "Fullscreen" below). Everything described in this section — backdrop, art regime, lyrics, the action row — is therefore true of both; only the sizing differs. Neither chrome draws its own title/artist/album block — the now-playing bar (in-grid) and `FullscreenControls` (fullscreen) each already carry the identity, so the view drawing it again was the same fact twice on one screen; the windowed copy was removed in issue #136 (it crowded 1080p displays). New behaviour added here lands in both places, which is the point.

**Audio tracks:** blurred art backdrop + foreground art + centered lyrics. Image resolution uses the album→artist `useImageCache` chain (same as queue/bar), through the pure `resolveNowPlayingSlides` (`utils/nowPlayingArt.ts`). Art presence switches the whole surface regime — backdrop + always-light text, or the `--noart` skin gradient + skin text — so that switch waits for a **settled** lookup (`useImageCache.isResolved`); an unsettled one holds the art regime over `--np-backdrop-base` instead of flipping the regime twice per new album.

**Art slideshow (issue #135):** when a track has both an album cover and a distinct artist image, the surface rotates between them every 20s (`NOW_PLAYING_SLIDE_INTERVAL_MS`), crossfading via the existing `Crossfade` — always on, no setting. The **backdrop follows the current slide** (one source of truth — foreground and ambience never disagree). Rules: rotation runs **only while playing** (a paused surface holds its slide, same reasoning as the idle-hide's `hold`); every **track change resets to the cover** (album first, keyed previous-value-in-state on `track.key`); a single image — or the artist lookup still in flight — is just the static art this view always showed, and the interval restarts when the second slide lands so the first flip is a full beat later; **reduce-motion holds the slideshow** (checked at tick time via `isReducedMotion()`, so the Settings toggle takes effect mid-song). A plugin visualizer replaces the art column but the backdrop still follows the slides behind it. `resolveNowPlayingArt` (cover with artist *fallback*, no always-ask) remains for App's `nowPlayingArtSrc` — the visualizer host's `currentArtUrl` is not a slideshow consumer.
...
**Type scales with the stage.** Lyrics (`--np-lyric-size`) are `clamp()`ed against `vh + vw` instead of being fixed at a `--fs-*` token, so a 4K display gets lyrics you can read from a sofa rather than the same 19px it showed on a laptop. Three rules, all load-bearing:
...
**Fullscreen:** `AudioFullscreen.tsx`, an opaque full-window overlay at the app root. Its surface is **this same view** at `variant="fullscreen"` — so fullscreen carries the blurred backdrop, the art regime, the corner buttons and, notably, the **lyrics**, which the old art-only fullscreen dropped (the more lean-back of the two surfaces was the one without karaoke). The stage is the `fullscreen` visualizer slot, which inherits the Now Playing pick (see `plugins.md` "Visualizers"), falling back to album art and then the initials placeholder — the view's own ladder, which is why fullscreen works for anything playing rather than only for users who installed a visualizer. Only sizing differs from the in-grid view: with the app chrome gone, the `.now-playing-view--fs` tier gives the stage the height and width budget the in-grid tier reserves for furniture that isn't there, and the title comes from the control bar. Window fullscreen, not DOM element fullscreen. Controls are the shared `FullscreenControls` on the usual idle fade; **Escape** or **Cmd/Ctrl+F** exits. Audio only — video has its own fullscreen path (it owns the shared `<video>` element and can't be re-parented).

**The bar keeps both buttons here.** `onToggleFullscreen` and `onToggleQueue` are both **optional** on `FullscreenControls`, and audio fullscreen now passes both. **Exit** — the restore control belongs at the right end of the control bar in *every* fullscreen, so it is where the windowed bar's fullscreen button just was. It does duplicate the surface's corner row, and that is accepted: the corner row fades with the artwork and reads as part of the presentation, while the bar is the transport. **Queue** opens the drawer (pressed state while it is up) — it was dropped for a while as "a second route to the edge gesture", and since that gesture is gone (issue #125) it is the *only* pointer route: nothing taught the gesture, issue #123's reporter found the drawer and still read it as broken, and a keyboard user could not perform it at all. In fullscreen **Cmd/Ctrl+P** toggles the same state (via `handleToggleQueueCollapsed`'s fullscreen branch — the persisted collapsed flag is never touched there). Video fullscreen passes both too (no corner row).

**Queue drawer (toggled, not hovered).** The root `.app` gains `audio-fs-open`, and `QueuePanel.css` turns the panel from a grid column into a fixed drawer: parked at `translateX(100%)`, `z-index: 1000` (above the overlay's 999, below the auto-continue popover at 1100 and modals at 2000), spanning `top: 52px` to `bottom: var(--now-playing-h)`. `App.tsx` holds one `fsQueueOpen` flag — live only while fullscreen is up, toggled by the bar's Queue button or Cmd+P — and the root gains `fs-queue-revealed`, which slides it in. The drawer paints **translucent** (`rgba(var(--bg-primary-rgb), 0.78)` + `backdrop-filter: blur`) rather than the grid panel's opaque `--bg-secondary`, so it reads as part of the fullscreen presentation instead of windowed chrome — still skin-token-driven, since an always-dark scrim would invert row text on light skins. On each open the panel scrolls to the playing track (the `fsRevealed` prop, folded into the existing scroll-on-uncollapse effect) — the parked drawer keeps its scroll position, so it would otherwise open on wherever the user last browsed.
...
- **No pointer-driven reveal, and don't add one back.** The drawer used to slide in whenever the pointer came within 24px of the right edge, hiding again only once it cleared the whole drawer (hysteresis, so a single boundary couldn't flicker it) and never while a mouse button was held (a row-reorder or width drag legitimately leaves the drawer). It still misfired constantly, because the right edge is the route to everything up there — the corner action row, the window controls — so one trip to a button opened and shut the drawer several times, and an invisible gesture gave the user nothing to blame. Removed in issue #125; the Queue button and Cmd+P are the whole interface.
- **`top: 52px`** clears `.np-actions`. Full-height slid the drawer over the visualizer / lyrics / exit buttons, so opening the queue appeared to delete them.
- **No width is reserved on the surface.** The drawer is transient, so it overlays; reserving space would slide the artwork sideways on every toggle. Persistent panels reserve, transient ones cover.
