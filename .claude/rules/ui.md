...
**Filmstrip seek bar** (`FilmstripSeekBar.tsx` / `.css`) — the seek surface for a video with a storyboard, on both the now-playing bar (34px) and `FullscreenControls` (28px). The frames *are* the content, so four rules hold and each one replaced a bug:
...
**View actions (top-right):** a dim row (`.np-actions`) that brightens on hover of the view, in the same corner the video theater puts its own controls so the set doesn't jump when the queue turns up a video. That pointer-in-view reveal is a *floor*, so its ancestor sits in `:where()` and carries no specificity — written plainly, `.now-playing-view:hover .np-action-btn` out-specifies `.np-action-btn:hover`, and since the pointer must be in the view to reach a button, a button could never reach its own hover opacity. The hover chip is `--np-btn-hover-bg`, which follows the regime (a fixed dark chip over scrimmed art, the skin's `--bg-hover` over the skin gradient) — the skin colour alone put a white glyph on a pale block in light skins. **Every action is a visible button — there is no ⋯ and no right-click menu.** Left to right:
...
**The bar drops two buttons here.** `onToggleFullscreen` and `onToggleQueue` are **optional** on `FullscreenControls`, and audio fullscreen passes neither, so no Exit and no Playlist button. Both actions are already on this surface by another route — the corner row toggles fullscreen, and the queue reveals itself at the right edge — so the buttons would be second routes to things already reachable. Video fullscreen still passes both (it has no corner row, and the Exit button is its only visible way out besides Escape).
