...
**Now Playing info line** (the line under the title, **mini player only** — the full bar shows a static Artist · Album line instead): a dynamic, auto-cycling section. Modules and plugins register items (`api.nowPlayingInfo` — see `plugins.md`); built-ins are **Artist · Album**, **Artist**, **Album**, **Plays · Rank** (one item), **Source**, **Quality**, **Duration**, **Tags** (`#`-prefixed track tags), **Synced Lyrics** (current LRC line in quotes, tracks playback position), **Plain Lyrics** (one unsynced line in quotes, stable per track), plus the Last.fm plugin's **Scrobbles**. The lyrics items reuse the cached lyrics info-type and each shows only when that kind of lyrics exists for the track. Each item carries a per-type **style** (lyrics italic, play-stats accent, secondary metadata muted — all via skin tokens, never hardcoded colors) and a **time-of-persistence** (ToP) multiplier, both set via the mini player's native right-click context menu. Each item is a submenu of `Off` / `Preview only` (0×) / `1×` / `2×` / `5×` / `10×`. The cycle runs two phases per track: a **preview pass** (every enabled item shown once at the base ~5s interval, with a slide animation) then **steady rotation** of the `top > 0` items ordered by ToP descending (largest first), each dwelling `5s × top`; `0×` items appear only in the preview. Rendered by `NowPlayingInfoCycler`; resolution/selection/styling live in `useNowPlayingInfo`; selection persists as `nowPlayingInfoSelection` and the ToP map as `nowPlayingInfoPersistence`. Each item declares its own default-enabled state (`defaultEnabled`); **Artist · Album** (1×), **Synced Lyrics** (5×, italic), and **Scrobbles** (0×) are on by default — the rest (including **Tags** and **Plain Lyrics**) are opt-in.
