...
| `playTidalTrack(track)` | Play a single TIDAL track | **tidal-browse** — plays selected track from search/album views |
| `enqueueTidalTrack(track)` | Enqueue a TIDAL track | **tidal-browse** — enqueues track from context menu/UI |
| `playTidalTracks(tracks, startIndex, context)` | Play multiple TIDAL tracks with playlist context | **tidal-browse** — plays album or search result list |
...
| `requestAction(action, payload)` | Request a host-level action (play-tracks, navigate, etc.) | **spotify-browse** — play-tracks, enqueue-tracks, navigate-to-artist/album; **tidal-browse** — play-tracks, enqueue-tracks, tidal-download-album, navigate-to-artist/album, show/hide-loading; **auto-tagger** — refresh-library after applying tags |
