...
| `priority` | number | Default provider priority when multiple plugins register the same ID (lower = tried first) |
...
- **`entity_key`**: Canonical string using existing `strip_diacritics` + `unicode_lower`. Format: `"artist_name"` for artists, `"artist_name::album_title"` for albums, `"artist_name::track_title"` for tracks, `"tag_name"` for tags.
- **`status`**: `"ok"` | `"not_found"` | `"error"`
- **`plugin_id`**: Which plugin provided this cached result
...
2. **Query cached values**: For each type, look up `information_values WHERE entity_key = 'radiohead'`
...
When cached data is stale, refetch starts from the provider that originally succeeded (stored `plugin_id`), falling through to others only if it now fails.
...
| `entity_list` | `{ items: [{ name, subtitle?, match?, image?, url?, libraryId? }] }` | List with optional match bars | Similar tracks, similar artists |
...
| `ranked_list` | `{ items: [{ name, subtitle?, value: number, maxValue?: number, libraryId? }] }` | Rows with popularity bars | Top tracks, track popularity |
...
7. **`lyrics`** — validates lyrics renderer, migrates off dedicated `lyrics` table
