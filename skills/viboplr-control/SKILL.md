...
| `POST /tracks/{id}/download` | needs **Download tracks**. `{collectionId, subdir?}` — downloads the track's **own** `subsonic://` or direct `http(s)` source, as itself, into `<local collection root>/<subdir>/Artist - Title.ext` and indexes it. Source-faithful: plugin-scheme tracks are refused here (use `/downloads/plugin`); never overwrites. Can run minutes — use a long timeout |
| `POST /downloads/plugin` | needs **Download tracks**. Downloads a plugin-sourced track — audio OR video — through the plugin that OWNS it, lands it in a local collection, indexes it, and writes the provider's metadata as file tags. Body: `{collectionId, subdir?}` + ONE addressing mode: `{searchId, index}` (a `/search/plugin` result), `{trackId}` (a plugin-scheme library track), `{uri}` (a plugin-scheme URI), or `{title, artistName?, albumTitle?, pluginId}` (metadata resolve — the plugin searches its source; `pluginId` REQUIRED, the app never picks a provider). `{listQualities: true, pluginId}` returns the provider's quality options instead; `quality` defaults to the FIRST declared option (audio), so a VIDEO download must pass a `video: true` option's value (yt-dlp: `video`, `video-720`, …). The resolve can BE the whole download (yt-dlp fetches + merges — minutes; use a 10-min timeout); one at a time; never overwrites. Confirm the pick with the user before downloading a catalog result |
| `DELETE /downloads/plugin` | cancel the in-flight plugin download resolve (kills the provider's subprocess) → `{cancelled, pluginId?}` |
