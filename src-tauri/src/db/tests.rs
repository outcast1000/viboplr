//! Unit tests for the Database core (see `db/mod.rs`).
//!
//! Extracted verbatim from the former inline `#[cfg(test)] mod tests` block in
//! `mod.rs` to keep that file focused on schema + query helpers. `super` still
//! resolves to the `db` module, so every `use super::*;` here is unchanged.
