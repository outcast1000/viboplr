...
function makeTrack(id: number): Track {
  return {
    id, key: `lib:${id}`, path: `file:///test${id}.mp3`, title: `Track ${id}`,
...
describe("computeSelection", () => {
  it("plain click selects only the clicked track", () => {
    const result = computeSelection(new Set(["lib:10", "lib:20"]), 2, tracks, null, false, false);
    expect(result).toEqual(new Set(["lib:30"]));
  });

  it("plain click on already-selected sole track is idempotent", () => {
    const result = computeSelection(new Set(["lib:30"]), 2, tracks, null, false, false);
    expect(result).toEqual(new Set(["lib:30"]));
  });

  it("meta+click toggles a track into the selection", () => {
    const result = computeSelection(new Set(["lib:10"]), 2, tracks, null, true, false);
    expect(result).toEqual(new Set(["lib:10", "lib:30"]));
  });

  it("meta+click toggles a track out of the selection", () => {
    const result = computeSelection(new Set(["lib:10", "lib:30"]), 2, tracks, null, true, false);
    expect(result).toEqual(new Set(["lib:10"]));
  });

  it("shift+click selects a range from lastIndex", () => {
    const result = computeSelection(new Set(), 3, tracks, 1, false, true);
    expect(result).toEqual(new Set(["lib:20", "lib:30", "lib:40"]));
  });
  ...
  it("shift+click with no lastIndex selects from 0 to clicked", () => {
    ...
    expect(result).toEqual(new Set(["lib:10", "lib:20", "lib:30"]));
  });
  ...
  it("meta+shift+click unions range with existing selection", () => {
    const result = computeSelection(new Set(["lib:10"]), 3, tracks, 2, true, true);
    expect(result).toEqual(new Set(["lib:10", "lib:30", "lib:40"]));
  });
  ...
  it("clicking last index works", () => {
    ...
    expect(result).toEqual(new Set(["lib:50"]));
  });
});
